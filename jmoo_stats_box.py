...
class jmoo_stats_box:
    ...
    def update(statBox, population, gen, num_new_evals, initial=False, population_size=None, printOption=True):
        ...
        if jmoo_properties.Configurations["Universal"]["hpc"]:
            filename = filename.replace("Data/results_", "HpcData/results_" + str(
                    jmoo_properties.Configurations["Universal"]["repeat_id"]) + "_")
        ...
        objective_columns = [[objective[i] for objective in objectives] for i, obj in
                             enumerate(statBox.problem.objectives)]
        ...
        norms = [[min(objective_columns[i] + [statBox.referencePoint[i]]),
                  max(objective_columns[i] + [statBox.referencePoint[i]])] for i, obj in
                 enumerate(statBox.problem.objectives)]

        lossInQualities = [{"qual": loss_in_quality(statBox.problem, [statBox.referencePoint], fit, norms), "index": i}
        ...
        if printOption is True:
            ...
            if initial:
                ...
                outString += str("%8.4f" % IBD) + "," + percentChange(statBox.referenceIBD, statBox.referenceIBD, True,
                                                                      0, 1) + "," + str("%8.4f" % IBS)
            else:
                outString += str(statBox.numEval) + ","
                for med, spr, initmed, obj, o in zip(best_fitness, objective_iqr, statBox.referencePoint,
                    ...
                    if statBox.numEval in statBox.foam[o]:
                        ...
                        statBox.foam[o][statBox.numEval] = [change]
                outString += str("%8.4f" % IBD) + "," + percentChange(IBD, statBox.referenceIBD, True, 0,
                                                                      1) + "," + str("%8.4f" % IBS)
            fa.write(outString + "\n")
        ...
        statBox.box[-1] = jmoo_stats(trunk, objectives, best_fitness, objective_iqr, statBox.numEval, gen, IBD, IBS,
                                     changes)
        fa.close()
...
def changeFromOld(new, old, lismore, low, up):
    ...
    if x == 0:
        ...
        return 100.0 * x ** (1 if ourlismore else -1)
